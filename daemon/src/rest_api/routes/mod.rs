...
mod test {
    ...
    fn validate_location_value(location_value: &StructPropertyValue, expected_value: LatLong) {
        ...
        assert_eq!(location_value.name, "LocationProperty".to_string());
        assert_eq!(location_value.value, Value::Location(expected_value));
    }
    ...
    fn validate_enum_value(enum_value: &StructPropertyValue, expected_value: i32) {
        ...
        assert_eq!(enum_value.value, Value::Enum(expected_value));
    }
    ...
    fn validate_bytes_value(bytes_value: &StructPropertyValue, expected_value: &[u8]) {
        ...
        assert_eq!(
            bytes_value.value,
            Value::Bytes(base64::encode(&expected_value))
        );
    }
    ...
    fn get_reported_value() -> Vec<NewReportedValue> {
        vec![
            ...
            NewReportedValue {
                start_block_num: 2,
                end_block_num: MAX_BLOCK_NUM,
                property_name: "TestProperty_LocationProperty".to_string(),
                record_id: "record_01".to_string(),
                reporter_index: 0,
                timestamp: 5,
