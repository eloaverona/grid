...
fn parse_value(
    ...
    match val.data_type.as_ref() {
        ...
        "Boolean" => {
            let boolean_value = val.boolean_value.clone().ok_or_else(|| {
                RestApiResponseError::DatabaseError(
        ...
        "Enum" => {
            let enum_value = val.enum_value.clone().ok_or_else(|| {
                RestApiResponseError::DatabaseError(
        ...
        "LatLong" => {
            ...
            Ok(Value::Location(lat_long))
        }
        "Number" => {
            let number_value = val.number_value.clone().ok_or_else(|| {
                RestApiResponseError::DatabaseError(
...
impl Handler<FetchRecordProperty> for DbExecutor {
    ...
    fn handle(&mut self, msg: FetchRecordProperty, _: &mut SyncContext<Self>) -> Self::Result {
        ...
        let mut updates = db::list_reported_value_with_reporter_and_metadata(
            &*self.connection_pool.get()?,
            &msg.record_id,
            &msg.property_name,
        )?
